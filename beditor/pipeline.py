...
import logging
from configure import get_genomes,validcfg,validinput,get_genomes_by_path
from lib.io_sys import runbashcmd
from lib.io_dfs import read_table,to_table
from lib.io_strs import get_datetime    
import yaml   
...
from lib.io_dfs import fhs2data_combo_appended
from lib.global_vars import stepi2name
from lib.io_nums import str2num
                        ...
                        for control_type in ['pos','neg']:
                            ...
                            if cfg[f'make_control_{control_type}']:
                                if not exists(dguides_controlp):
                                    from output import collectchuckfiles
                                    from lib.global_vars import stepi2colsoutput
...
from lib.plot_res import plot_vizbysteps
def make_outputs(cfg,plot=True):
    ...
    if not exists(doutputp) or cfg['force']:
        ...
        if len(doutput)!=0 and 'guide+PAM sequence' in doutput:
            from lib.io_seqs import get_polyt_length
...
def pipeline(cfgp,step=None,test=False,force=False):
    ...
    cfg['step2ignoredl']=[2,3]
    if not cfg['step'] in cfg['step2ignoredl']:
        # cfg=get_genomes(cfg)                                                   #选择上传还是下载的方式
        cfg = get_genomes_by_path(cfg)  
    #datads
    cfg[0]=cfg['prjd']+'/00_input/'
    cfg[1]=cfg['prjd']+'/01_sequences/'
...
#                       default_flow_style=False
    ...
    if (not '/chunk' in cfgp) and (step==1 or (step is None)):
        from lib.io_dfs import df2chucks
...
def main():
    ...
    if not args.cfg is None:
        from lib.io_strs import get_logger
        if args.test:
            level=logging.INFO
        else: 
            level=logging.ERROR
