...
import subprocess
import logging
from lib.io_sys import runbashcmd    
from lib.io_dfs import * 
...
def get_genomes_by_path(cfg):
    host_="_".join(s for s in cfg['host'].split('_')).capitalize()
    ncbi_fastad = 'pub/{}/fasta/{}/dna/'.format(cfg['genome_release'],cfg['host'])
    genome_fastad = '{}/{}'.format(dirname(realpath(__file__)),ncbi_fastad)
    cfg['genomep']='{}genome.fna'.format(genome_fastad)
    genome_fna_path = cfg['genome_fna_path']
    ...
    if not exists(cfg['genomep']): 
        ...
        runbashcmd(cmd,test=cfg['test'])
        # if not exists(cfg['genomep']):
        #     #解压文件夹中文件，并且将几个文件合并成一个文件
        #     cmd='unzip {}*.zip;cat {}/*.fna > {}/genome.fna;'.format(genome_fastad,genome_fastad,genome_fastad)
        #     runbashcmd(cmd,test=cfg['test'])
    ...
    if not exists(cfg['genomep']+'.sizes'):
     ...
     #download gff3
    ncbi_gff3d='pub/{}/gff3/{}/'.format(cfg['genome_release'],cfg['host'])
    genome_gff3d=f'{dirname(realpath(__file__))}/{ncbi_gff3d}'
    cfg['genomegffp']='{}/genome.gff'.format(genome_gff3d)
    genome_gff_path = cfg['genome_gff_path']  
    if not exists(cfg['genomegffp']):
        if not exists(genome_gff3d):
            os.makedirs(f'{genome_gff3d}')
        gff = cfg['genomegffp']
        cmd=f"cp -f {genome_gff_path} {gff}"
        runbashcmd(cmd,test=cfg['test'])  
    return cfg
...
def validcfg(cfg,outcfg=False): 
    ...
    :param cfg: configuration dict
    """
    from lib.global_vars import cfgoption2allowed,cfgoption2reguired
    opt_validity=[]
...
def validinput(cfg,din): 
    ...
    """
    from lib.global_vars import mutation_format2cols    
    opt_validity=[]
...
from lib.global_vars import cols_dpam
from lib.io_seqs import reverse_complement_multintseq,reverse_complement_multintseqreg,str2seq
from lib.io_dfs import *
from lib.io_strs import s2re
from lib.global_vars import multint2reg,multint2regcomplement,nt2complement
